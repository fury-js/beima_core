...
module.exports = async (callback) => {
    try {
			...
			// register user
			// result = await pensionContract.register(userDetails, {
			// 	from: account,
			// });
			...
			// console.log(result)
			result = await usdc.methods.approve(PensionServiceProvider.address, approvedAmountToSpend).send({from: account})
			// console.log(result)


			result = await pensionContract.deposit(cUSDC, amountToSpend);
			console.log(result.logs[0].args.amountSpent.toString())

			result = await pensionContract.pensionServiceApplicant(account)
			console.log(result)

			result = await pensionContract.withdraw();
			console.log(result)
